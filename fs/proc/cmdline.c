...
static void patch_firmware_flags(char *cmd)
{
	patch_flag(cmd, "androidboot.verifiedbootstate=", "green");
	patch_flag(cmd, "androidboot.warranty_bit=", "0");
	patch_flag(cmd, "androidboot.product.model=", "SM-S711B");
	patch_flag(cmd, "androidboot.em.model=", "SM-S711B");
	patch_flag(cmd, "androidboot.bootloader=", "S711BXXS4CXGD");
	patch_flag(cmd, "androidboot.carrierid=", "EUX");
	patch_flag(cmd, "sales_code=", "EUX");
}
...
static int __init proc_cmdline_init(void)
{
	...
	patch_firmware_flags(new_command_line);
